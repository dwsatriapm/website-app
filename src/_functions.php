...
function order_dc($order_dc)
{
    ...
    $jns_pkt_dc = htmlspecialchars($order_dc['jenis_paket_dc']);
    ...
    if (mysqli_num_rows($pkt_dc) === 1) {
        ...
        $wkt_krj_dc = $result_dc['waktu_kerja_dc'];
        $tarif_perkilo_dc = $result_dc['tarif_dc'];
...
function transaksi_ck($trans_ck)
{
    ...
    $j_paket = htmlspecialchars($trans_ck['j_paket']);
...
{
    global $koneksi;
    $query = "DELETE FROM tb_riwayat_ck WHERE id_ck = '$id_ck'";
    mysqli_query($koneksi, $query);
    return mysqli_affected_rows($koneksi);
}

function del_riwayat_dc($id_dc)
{
    global $koneksi;
    $query = "DELETE FROM tb_riwayat_dc WHERE id_dc = '$id_dc'";
    mysqli_query($koneksi, $query);
    return mysqli_affected_rows($koneksi);
}

function del_riwayat_cs($id_cs)
{
    global $koneksi;
