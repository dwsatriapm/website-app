               ...
               <?php if (!empty($query_ck)) : ?>
                  ...
                  <?php foreach ($query_ck as $data_ck) : ?>
                     <tr>
                        ...
                        <td align="center">
                           <a href="<?= url('riwayat_transaksi/riwayat_ck/detail_ck.php') ?>?id_ck=<?= $data_ck['id_ck'] ?>" class="btn btn-detail">Detail</a><br />
                           <a href="<?= url('riwayat_transaksi/riwayat_ck/cetak_info.php') ?>?id_ck=<?= $data_ck['id_ck'] ?>" class="btn btn-edit" style="color: whitesmoke;">Cetak Bukti</a>
                           <a href="<?= url('riwayat_transaksi/hapus_riwayat.php?type=ck&id=' . $data_ck['id_ck']) ?>"
                              class="btn btn-hapus"
                              onclick="return confirm('Yakin ingin menghapus riwayat transaksi ini?\n\nNo Order: <?= $data_ck['or_number'] ?>\nPelanggan: <?= $data_ck['pelanggan'] ?>')">
                              Hapus
                           </a>
                        </td>
                     </tr>
